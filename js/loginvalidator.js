$(window).on("load", function() {
    $('.pictitle').each(function(){
        $(this).css({"display":"block"});
    });

    $('.quiztitle').each(function(){
        $(this).css({"display":"block"});
    });
});
...
$(document).ready(function (e) {
    ...
    $('#colname').change(function () {
        if ($('#colname').val() == "Other")
            $('#college').stop().slideDown();
        else $('#college').stop().slideUp();

    });

    $('#country').change(function () {
        if ($('#country').val() == "Other")
            $('#countryo').stop().slideDown();
        else $('#countryo').stop().slideUp();

        if ($('#country').val() == "India")
            $('#statesindia').stop().slideDown();
        else
            $('#statesindia').stop().slideUp();

    });

    $('#hear').change(function () {
        if ($('#hear').val() == "Other")
            $('#hearother').stop().slideDown();
        else $('#hearother').stop().slideUp();
    });
    ...
    $("#closesignin").click(function (e) {
        $("#signinpopupwrap").stop().slideToggle('fast');
        $("#signinemail").blur();
    });
    ...
    $("#signin-link").click(function (e) {
        ...
        if($("#signinpopupwrap").is(':visible'))
            $("#signinemail").focus();
        return false;
    });


    $("#myRegisterButton").click(function (e) {
        $("form.registerForm").slideToggle('fast');
        $("#myRegisterButton").toggleClass('selected');
    ...
    $('div form.registerForm').validate({
        errorElement: 'label',
        errorClass: 'error',
        rules: {
            ...
            'countryo':   { required: true,},
            'statesindia':{ required: true,},
            'city':       { required: true,},
            'phone3':     { required: true, phoneno: true,},
            'colname':    { required: true,},
            'college':    { required: true,},
            'hear':       { required: true,},
            'hearother':  { required: true,},
        },
        messages: {
            'email':     { email: "Please enter a valid email!",},
            'password':  { minlength: "Password must be at least 6 characters",},
            'passwordc': { equalTo: "Passwords don't match",},
        }
    });

    $('div form.signinForm').validate({
        errorElement: 'label',
        errorClass: 'error',
        rules: {
            'email': { required: true,},
